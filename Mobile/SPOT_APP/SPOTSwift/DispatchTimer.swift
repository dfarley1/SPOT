...
class DispatchTimer: NSObject {
  ...
  init(delay: TimeInterval, queue: DispatchQueue, block: TimerHandler) {
    ...
    self.source = DispatchSource.makeTimerSource(flags: 0, queue: self.queue) /*Migrator FIXME: Use DispatchSourceTimer to avoid the cast*/ as! DispatchSource

    super.init()

    let wrapper = { () -> Void in
      if self.source.isCancelled == 0 {
        self.source.cancel()
